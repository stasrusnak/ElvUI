...
-- Тег для иконки элитки или босса с размером 16x16
ElvUF.Tags.Methods["isbossicon20"] = function(unit)
	local classification = UnitClassification(unit)
	if classification == "elite" or classification == "rareelite" or UnitLevel(unit) == -1 then
	    return "|TInterface\\TargetingFrame\\UI-TargetingFrame-Elite:20:20|t" -- Иконка элитки (дракон)
	elseif classification == "worldboss" or classification == "rare" then
	    return "|TInterface\\TargetingFrame\\UI-TargetingFrame-Skull:20:20|t" -- Иконка босса (череп)
	else
	    return "" -- Пустая строка, если цель не является ни элитной, ни боссом
	end
  end
 
  ElvUF.Tags.Methods["isbossicon40"] = function(unit)
	local classification = UnitClassification(unit)
	if classification == "elite" or classification == "rareelite" or UnitLevel(unit) == -1 then
	    return "|TInterface\\TargetingFrame\\UI-TargetingFrame-Elite:40:40|t" -- Иконка элитки (дракон)
	elseif classification == "worldboss" or classification == "rare" then
	    return "|TInterface\\TargetingFrame\\UI-TargetingFrame-Skull:40:40|t" -- Иконка босса (череп)
	else
	    return "" -- Пустая строка, если цель не является ни элитной, ни боссом
	end
  end
  ...
  ElvUF.Tags.Methods["isbossicon200"] = function(unit)
	local classification = UnitClassification(unit)
	if classification == "elite" or classification == "rareelite" or UnitLevel(unit) == -1 then
	    return "|TInterface\\TargetingFrame\\UI-TargetingFrame-Elite:200:200|t" -- Иконка элитки (дракон)
	elseif classification == "worldboss" or classification == "rare" then
	    return "|TInterface\\TargetingFrame\\UI-TargetingFrame-Skull:200:200|t" -- Иконка босса (череп)
	else
	    return "" -- Пустая строка, если цель не является ни элитной, ни боссом
	end
  end